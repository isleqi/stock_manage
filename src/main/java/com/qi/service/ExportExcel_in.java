...
public class ExportExcel_in {
	...
	public void export (String[] title,List data,String fileName) throws IOException {
		...
		In_danju datamap=(In_danju) it.next();
            ...
            // 输出数据  
                int j=0;
                cell = row.createCell(j++);  
                cell.setCellValue(new HSSFRichTextString(String.valueOf(datamap.getIn_dnum())));  
                cell = row.createCell(j++);  
                ...
                cell = row.createCell(j++);  
                cell.setCellValue(new HSSFRichTextString(String.valueOf(datamap.getIn_person_info().getIn_person())));  
                cell = row.createCell(j++);
                cell.setCellValue(new HSSFRichTextString(String.valueOf(datamap.getIn_person_info().getIn_company())));  
                cell = row.createCell(j++);
                cell.setCellValue(new HSSFRichTextString(String.valueOf(datamap.getIn_person_info().getTel()))); 
                cell = row.createCell(j++);
